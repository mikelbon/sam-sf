"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const confirmarTransaccion_1 = require("../../src/lambdas/confirmarTransaccion");
const aws_sdk_client_mock_1 = require("aws-sdk-client-mock");
const client_dynamodb_1 = require("@aws-sdk/client-dynamodb");
const ddbMock = (0, aws_sdk_client_mock_1.mockClient)(client_dynamodb_1.DynamoDBClient);
beforeEach(() => ddbMock.reset());
describe("ConfirmarTransaccionFunction", () => {
    beforeEach(() => ddbMock.reset());
    it("confirma transacción válida y registra en DynamoDB", async () => {
        ddbMock.on(client_dynamodb_1.PutItemCommand).resolves({});
        const event = {
            Payload: {
                referencia: "TX123",
                medio: "tarjeta",
                estado: "aprobado", // ✅ literal
                usuarioId: "U456",
            },
        };
        const result = await (0, confirmarTransaccion_1.handler)(event);
        expect(result.Payload.confirmado).toBe(true);
        expect(result.Payload.referencia).toBe("TX123");
        expect(result.Payload.medio).toBe("tarjeta");
        expect(result.Payload.timestamp).toMatch(/\d{4}-\d{2}-\d{2}T/);
        const calls = ddbMock.commandCalls(client_dynamodb_1.PutItemCommand);
        expect(calls.length).toBe(1);
        expect(calls[0].args[0].input).toEqual(expect.objectContaining({
            TableName: "Transacciones",
            Item: expect.objectContaining({
                referencia: { S: "TX123" },
                medio: { S: "tarjeta" },
                estado: { S: "aprobado" },
                usuarioId: { S: "U456" },
            }),
        }));
    });
    it("lanza error si el estado no es aprobado", async () => {
        const event = {
            Payload: {
                referencia: "TX999",
                medio: "yape",
                estado: "rechazado",
                usuarioId: "U789",
            },
        };
        await expect((0, confirmarTransaccion_1.handler)(event)).rejects.toThrow("Transacción fallida");
    });
});
