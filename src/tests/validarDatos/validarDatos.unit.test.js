...
const validarDatos_1 = require("../../src/lambdas/validarDatos");
describe("ValidarDatosFunction", () => {
    it("retorna validado si el payload es correcto", async () => {
        ...
        const result = await (0, validarDatos_1.handler)(event);
    ...
    it("lanza error si el monto es inválido", async () => {
        ...
        await expect((0, validarDatos_1.handler)(event)).rejects.toThrow("Monto inválido");
    });
    it("lanza error si el medio no es soportado", async () => {
        ...
        await expect((0, validarDatos_1.handler)(event)).rejects.toThrow(/Medio de pago no soportado/i);
    });
    it("lanza error si el usuarioId está vacío", async () => {
        ...
        await expect((0, validarDatos_1.handler)(event)).rejects.toThrow(/Usuario no identificado/i);
    });
});
